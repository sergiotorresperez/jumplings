...
public class InAppPurchaseHelper {
	 ...
	 * @param listener
	 */
	public void queryIsPremiumPurchasedAsync(final Context context,
			final PurchaseStateQueryCallback listener) {
		if (!mIsSetUp) {
		...
		try {
			...
			mHelper.queryInventoryAsync(new QueryInventoryFinishedListener() {
				...
				public void onQueryInventoryFinished(IabResult result,
						Inventory inv) {
					if (!result.isSuccess()) {
						LogX.e(TAG, "Querying inventory resulted in error: "
								+ result.getMessage());
						if (listener != null) {
							listener.onPurchaseStateQueryError("Error querying user inventory: "
									+ result.getMessage());
						}
					} else {
						...
						LogX.i(TAG,
								"Querying inventory finished. Is purchased: "
										+ isPurchased);
			...
			Log.e(TAG, "Error querying purchases purchase", t);
	 ...
	 * @param listener
	 */
	public void purchasePremiumAsync(final Activity activity,
			final PurchaseCallback listener) {
		if (!mIsSetUp) {
			LogX.d(TAG, "Starting setup.");
			mHelper.startSetup(new IabHelper.OnIabSetupFinishedListener() {
				@Override
			...
			mHelper.launchPurchaseFlow(activity, mNoAdsSKU, RC_REQUEST,
					new OnIabPurchaseFinishedListener() {
						...
						public void onIabPurchaseFinished(IabResult result,
							...
							if (!result.isSuccess()) {
								LogX.e(TAG,
										"Purchase process finished with error: "
												+ result.getMessage());
								if (listener != null) {
									listener.onPurchaseError("Error purchasing item: "
											+ result.getMessage());
								}
							} else {
								boolean isPurchased = false;
								...
								LogX.i(TAG,
										"Purchase process finished. Is purchased: "
												+ isPurchased);
								PermData.setPremiumPurchased(activity,
										isPurchased);
								if (listener != null) {
	...
	public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
		try {
			LogX.d(TAG, "onActivityResult(" + requestCode + "," + resultCode
					+ "," + data);
			...
			Log.e(TAG, "Error in onActivityResult after purchase", t);
			Crashlytics.logException(t);
		}
