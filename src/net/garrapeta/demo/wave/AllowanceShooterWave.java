...
import net.garrapeta.demo.Gun;
import net.garrapeta.demo.actor.BladePowerUpActor;
import net.garrapeta.demo.actor.BombActor;
import net.garrapeta.demo.actor.DoubleEnemyActor;
import net.garrapeta.demo.actor.EnemyActor;
import net.garrapeta.demo.actor.MainActor;
import net.garrapeta.demo.actor.LifePowerUpActor;
import net.garrapeta.demo.actor.RoundEnemyActor;
import net.garrapeta.demo.actor.SplitterEnemyActor;
...
public class AllowanceShooterWave extends AllowanceWave {
	...
	public AllowanceShooterWave(JumplingsGameWorld jgWorld, IWaveEndListener listener, int level) {
		...
		generatePowerUpAction = new RealTimeWaveAction(this) {
			...
			public void run() {
				generatePowerUp();
				this.schedule(getPowerUpCreationLapse());
			}
		};
		
		generatePowerUpAction.schedule(getPowerUpCreationLapse());
	...
	protected boolean isFinished() {
		return getProgress() >= 100 && jgWorld.jumplingActors.size() == 0;
	}
	...
	private double tryToCreateJumper(double threatNeeded, PointF initPos, Vector2 initVel) {
		...
		if (threat <= threatNeeded) {
			...
			case DoubleEnemyActor.JUMPER_CODE_DOUBLE:
				// double
				mainActor = new DoubleEnemyActor(jgWorld,  
			                 initPos);
				break;
	...
	private void generatePowerUp() {
		...
		// la probabilidad de que salga una vida crece conforme se ha perdido vida
