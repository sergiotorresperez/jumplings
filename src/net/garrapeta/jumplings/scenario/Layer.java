...
class Layer {
	...
	synchronized void setProgress(float progress) {
		if (progress <= 100) {
			float aux = maxHeight - scenario.mWorld.mView.getHeight();
	...
	public void draw(Canvas canvas, Paint paint) {
		int neededWitdh  = scenario.mWorld.mView.getWidth();
		int neededHeight = scenario.mWorld.mView.getHeight();
		...
		if (tileHorizontally) {
			xPosAux = xPosAux % scenario.mWorld.mView.getWidth();
		}
		if (tileVertically) {
			yPosAux = yPosAux % scenario.mWorld.mView.getHeight();
		}		
